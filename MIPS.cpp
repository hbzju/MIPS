...
void inst_char_to_int(const string& code);
...
unsigned int inst_char_to_int(const string& code)
{
	int i=0;
	unsigned int instruction=0;
	while(code[i]>'9'||code[i]<'0')i++;
	while(code[i]<='9'&&code[i]>='0')instruction=instruction*10+code[i]-'0';
	return instruction;
}

void Find_op_name(unsigned int instruction)
{
	unsigned int opvalue=instruction>>26;
	unsigned int opfunction;
	int index,flag=0;
	if(opvalue==0){//R type
		opfunction=(instruction<<27)>>27;
		//get instruction[0-5]
		...
		Re_JorR_type(instruction,index);
	}
}
...
void void Re_JorI_type(unsigned int& instruction,int index)
{
	int reg1,reg2,reg3,num1,num2;
	if (index <= 7){//load and sw
